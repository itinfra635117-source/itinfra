...
def admin_dashboard():
    ...
    # Fetch users
    url_users = f"{SUPABASE_URL}/rest/v1/users"
    resp_users = requests.get(url_users, headers=SUPABASE_HEADERS)
    users = resp_users.json() if resp_users.status_code == 200 else []
    return render_template('admin_dashboard.html', stats=stats, requests=filtered_requests, users=users)
